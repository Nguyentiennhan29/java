...
public class TestValidator {
    ...
    // ── Normalize output trước khi so sánh (giống LeetCode / Codeforces) ──
    // Xử lý: \r\n vs \n, trailing spaces, trailing newlines, dòng trống thừa
    private static String norm(String s) {
        if (s == null) return "";
        return Arrays.stream(s.replace("\r\n", "\n").replace("\r", "\n").split("\n"))
                .map(String::trim)                        // trim từng dòng
                .filter(line -> !line.isEmpty())          // bỏ dòng trống thừa
                .collect(Collectors.joining("\n"))
                .trim();
    }

    // So sánh output chuẩn hóa
    private static boolean judgeEqual(String expected, String actual) {
    ...
    public static Report validate(String code, String langStr, String codeType,
        ...
        if (useChecker && checkerCode != null && !checkerCode.isBlank()) {
            try {
                ...
                if (!che.isEmpty()) {
                    checkerDir = null;
                    if (log != null) log.accept("⚠ Checker CE, dùng so sánh mặc định");
                }
        ...
        for (int i = 0; i < tcs.size(); i++) {
            ...
            String v;
            if (res.status == CompilerRunner.Result.Status.TLE) {
                v = "TLE"; rep.tle++;
            } else if (res.status == CompilerRunner.Result.Status.RE) {
                v = "RE"; rep.re++;
            } else {
                boolean correct;
                if (checkerDir != null) {
                    try {
                        String cr = CompilerRunner.runChecker(lang, checkerDir,
                                tc.getInput(), tc.getExpected(), res.output);
                        correct = cr.startsWith("AC");
                    } catch (Exception e) {
                        // Fallback: dùng norm() nếu checker crash
                        correct = judgeEqual(tc.getExpected(), res.output);
                    }
                } else {
                    // FIX: Dùng judgeEqual() thay vì tc.isCorrect()
                    // tc.isCorrect() so sánh raw string → sai khi có \r\n, trailing space...
                    correct = judgeEqual(tc.getExpected(), res.output);
                }
